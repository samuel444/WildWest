...
def game():
    ...
    killFont = pygame.font.Font('assets/fonts/stencilfont.ttf', 55)
    ...
    finished = False
    while not finished:
        killText = killFont.render(str(kills), False, white)
        spawnTimer += 1
        ...
        for j in range(len(locations)):
            ...
            if locations[j][6]:
                locations[j][7] += 1
                if locations[j][7] >= 5 and locations[j][7] < 25:
                    if locations[j][2] == 'l':
                        look = rogueAttack
                        locations[j][0] -= 60
